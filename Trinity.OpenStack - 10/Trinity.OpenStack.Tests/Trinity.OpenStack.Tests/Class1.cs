...
namespace Trinity.OpenStack.Tests
{
    ...
    public class TestDeleteEndpoint
    {
        ...
        {
            if (Test_Delete_Endpoint_List(ref em, admin_token, admin_url, admin_token, iterationNumber))
            {
                int i = 0;
                Endpoint endp = em[i];
                if (DisposableEndpoints.Contains(endp))
                {
                    endp.Delete_Endpoint(admin_url, admin_token);
                }
                else
                {
                    while (em.Count > BaseCount && !DisposableEndpoints.Contains(endp))
                    {
                        i += 1;
                        endp = em[i];
                    }
